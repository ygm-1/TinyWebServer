...
const char* error_400_title = "Bad Requset";
...
void http_conn::initmysql_result(connection_pool* connPool) {
    ...
    // 在user表中检索账户密码在浏览器端输入
    if (mysql_query(mysql, "SELECT username, passwd FROM user")) {
        LOG_ERROR("LELECT error:%s\n", mysql_error(mysql));
    }
    //从表中检索的完整结果集
...
//从状态机， 分析一行
//返回值为读取状态 LINE_OK, LINE_BAD, LINE_OPEN

http_conn::LINE_STATUS http_conn::parse_line() {
    char temp;
    //checked_idx指向buffer中正在解析的字节， read_idx指向bufferz中客户数据尾部的下一个字节，
    //buffer中0~checked_idx都已经解析完毕
    //checked_idx~read_idx - 1由循环遍历分析
    for (; m_checked_idx < m_read_idx; m_checked_idx++) {
        //获得当前字符
        temp = m_read_buf[m_checked_idx];
        //获得回车符 可能是完整的行
        if (temp == '\r') {
            //如果"\r"是当前buffer中最后一个已被读入的数据，则没有分析到完整行
            //返回LINE_OPEN继续读去数据
...
//非阻塞ET工作模式下要一次性读完READ_BUFFER_SIZE
...
//解析http请求行， 获得请求方法， 目标url 及http版本号
http_conn::HTTP_CODE http_conn::parse_requset_line(char* text) {
    ...
    *m_url++ = '\0';
    ...
    m_url += strspn(m_url, " \t");
    m_version = strpbrk(m_url, " \t"); // 返回第一个" \t"的位置指针
    ...
    *m_version++ = '\0';
    ...
    if (!m_url || m_url[0] != '/') return BADREQUEST;
    //当url为/时显示判断界面
...
http_conn::HTTP_CODE http_conn::parse_headers(char* text) {
    ...
    //处理头部字段
        ...
        text += 11;
        text += strspn(text, " \t"); // 返回 text中与" \t" 相同的个数 
        ...
        text += 15;
        text += strspn(text, " \t");
        m_content_length = atol(text); //字符转为long 后面字母忽略
    }
...
http_conn::HTTP_CODE http_conn::process_read() {
    ...
    while ((m_check_state == CHECK_STATE_CONTENT && line_status == LINE_OK) || ((line_status = parse_line()) == LINE_OK)) {
        ...
        LOG_INFO("%s", text);
        //m_check_state记录了主状态机的状态
        switch(m_check_state) {
            case CHECK_STATE_REQUESTLINE: {
                ret = parse_request_line(text);
                if (ret == BAD_REQUSET) return BAD_REQUSET;
                break;
            }
            ...
            case CHECK_STATE_CONTENT: {
                ...
                if (ret == GET_REQUSET) return do_request();
...
http_conn::HTTP_CODE http_conn::do_request() {
    ...
    const char* p = strrchr(m_url, '/'); // 从后面开始第一次出现'/' 返回及其后面的字符
    ...
    if(cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3')) {
        ...
        //登录直接判断
            ...
            if (users.find(name) != users.end() && user[name] == password) 
...
bool http_conn::write() {
    ...
    while(1) {
        ...
        if (temp < 0) {
            //如果TCP写缓冲没有空间则等待下一轮EPILLOUT事件，保持连接的完整性
            if (errno == EAGAIN) {
